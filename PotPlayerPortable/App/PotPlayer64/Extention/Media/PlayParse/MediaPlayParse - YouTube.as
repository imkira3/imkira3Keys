...
string GetFunction(string str)
{
	...
	ret = HostRegExpParse(str, "\\.sig\\|\\|([a-zA-Z0-9$]+)\\(");
	if (!ret.empty()) return ret;
	
	ret = HostRegExpParse(str, "\\bc\\s*&&\\s*d\\.set\\([^,]+\\s*,\\s*([a-zA-Z0-9$]+)\\(");
	if (!ret.empty()) return ret;

	ret = HostRegExpParse(str, "\\bc\\s*&&\\s*d\\.set\\([^,]+\\s*,\\s*\\([^)]*\\)\\s*\\(\\s*([a-zA-Z0-9$]+)\\(");
	if (!ret.empty()) return ret;
...
//				if (@MetaData !is null) MetaData["chatUrl"] = "https://www.youtube.com/live_chat?v=" + videoId;
			...
			string player_responce_jsonData;
			if (isAPI) player_responce_jsonData = HostUrlDecode(GetEntry(strData, MATCH_PLAYER_RESPONSE2, MATCH_END2));
			else player_responce_jsonData = HostUrlDecode(GetEntry(strData, MATCH_PLAYER_RESPONSE, MATCH_PLAYER_RESPONSE_END));			
			...
			if (!final_url.empty())
			{
				...
				if (!videoId.empty() && (@MetaData !is null))
				{
					...
					if (@QualityList !is null)
					{
						...
						if (!player_responce_jsonData.empty())
						{
							player_responce_jsonData = player_responce_jsonData + "}";
							...
							if (Reader.parse(player_responce_jsonData, Root) && Root.isObject())
							{
